...
\newenvironment{ingredient}
  {
  \noindent\begingroup\edef
  \x{\endgroup\noexpand
  % \includegraphics[width=1\linewidth,height=2.5cm,clip,viewport=\@ImageDim]{\@ImagePath}
  }\x
  \maketitle
  
  \begin{footnotesize}\noindent
  \begin{tabular}{|L{0.62\linewidth}|L{0.33\linewidth}|}
    \hline\vspace{-0.21cm}\underline{\textbf{{\normalsize Ингридиенты (\@NbPerson порции):}}} &\\
    ...
    \end{minipage}
    & 
    % \vspace{-1.8cm}Необходимо времени: \PrepTimeb~мин\par
    % \vspace{0.2cm} Cooking: \@CookingTime min -- \@CookingTempe° \par 
    % \vspace{0.2cm} Type of cooking: \@TypeCooking 
    \\
    \hline
  \end{tabular}
  \vspace{0.5cm} 
...
\noindent\underline{\textbf{Заметки:}}%
...
\newcommand{\step}[1]{\ifthenelse{\value{step}=0}{\noindent{\large \underline{\textbf{Процесс:}}}\vspace{0.3cm}
